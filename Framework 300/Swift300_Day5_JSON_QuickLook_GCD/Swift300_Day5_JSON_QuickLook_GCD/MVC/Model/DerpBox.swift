...
class Derpbox {
    ...
    func dados(vindoDe endpoint:String, body:[String:String]? = nil) -> Data? {
    ...
    func sobreEU(_ closure:(Usuario?) -> ()) {
        
        // Essa é a localização na internet da informação
        // de quem sou eu, no DropBox
        let endpoint = "\(enderecoBase)/users/get_current_account"
    ...
    func listarPasta(comCaminho caminho:String = "", depois closure:([Conteudo])->()) {
        
        let endPoint = "\(enderecoBase)/files/list_folder"
        
        // Primeiro baixamos os bytes do endpoint
        guard let data = dados(vindoDe: endPoint,body: ["path":caminho]) else{
